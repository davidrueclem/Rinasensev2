...
extern RsQueue_t *xNetworkEventQueue = NULL;
 ...
 * @return pdPASS if the event was sent (or the desired effect was achieved). Else, pdFAIL.
 */
bool_t xSendEventStructToShimIPCPTask(const ShimTaskEvent_t *pxEvent, useconds_t xTimeOutUS)
{
    ...
    {
        LOGE(TAG_SHIM, "SHIM IPCP Task Not Ready, cannot send event");
    ...
    else
    {
        ...
        if (xSendMessage)
        {
             ...
             * itself to respond. */
            if ((xIsCallingFromShimIpcpTask()) && (xCalculatedTimeOutUS > 0))
                xCalculatedTimeOutUS = 0;

            /*#if SHIM_802154_MODULE
                        if (pxEvent->eEventType == eNetworkRxEvent)
                        {
                            BaseType_t task = false;
                            xReturn = xQueueSendToBackFromISR(xNetworkEventQueue, pxEvent, &task);
                        }
                        else
                        {
            #endif*/
            xReturn = xRsQueueSendToBack(xNetworkEventQueue, pxEvent, sizeof(ShimTaskEvent_t), xCalculatedTimeOutUS); /*Chance interruption*/
                                                                                                                      /*#if SHIM_802154_MODULE
                                                                                                                                  }
                                                                                                                      #endif*/
            if (!xReturn)
...
{
    ShimTaskEvent_t xReceivedEvent;
    ...
    for (;;)
    {
         ...
         * 'NoEvent' value. */
        if (xRsQueueReceive(xNetworkEventQueue, (void *)&xReceivedEvent, sizeof(ShimTaskEvent_t), xSleepTimeUS) == false)
        ...
        case eNetworkUpEvent:
            /*If coordinator wait, else send broadcast packet*/

            // vIeee802154_Broadcast(ieee802154_PANID_DESTINATION);
            break;

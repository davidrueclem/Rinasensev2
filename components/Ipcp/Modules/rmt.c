...
static rmtN1Port_t *pxRmtN1PortCreate(portId_t xId, struct ipcpInstance_t *pxN1Ipcp)
{
	...
	LOGI(TAG_RMT, "Creating a N-1 Port in the RMT, %pk", pxN1Ipcp);
	rmtN1Port_t *pxTmp;
...
bool_t xRmtN1PortBind(struct rmt_t *pxRmtInstance, portId_t xId, struct ipcpInstance_t *pxN1Ipcp)
{
	...
	pxRmtInstance->pxN1Port = pxTmp;
	pxRmtInstance->pxN1Port->eState = 1;

	LOGI(TAG_RMT, "Added send queue to rmt instance %pK for port-id %u", pxRmtInstance, xId);
	LOGI(TAG_RMT, "rmt instance %pK port-id state %d", pxRmtInstance, pxRmtInstance->pxN1Port->eState);
								...
								struct du_t *pxDu)
{
	...
	if (pxN1Port->pxN1Ipcp->pxOps->duWrite == NULL)
	{

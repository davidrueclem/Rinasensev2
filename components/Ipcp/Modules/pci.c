...
#define PCI_GETTER_NO_DTC(pci, pci_index, size, type)                                  \
	{                                                                                  \
		...
		{                                                                              \
		case (1):                                                                      \
			return (type) * ((uint8_t *)(pci->h + cfg->pci_offset_table[pci_index]));  \
		case (2):                                                                      \
			return (type) * ((uint16_t *)(pci->h + cfg->pci_offset_table[pci_index])); \
		case (4):                                                                      \
			return (type) * ((uint32_t *)(pci->h + cfg->pci_offset_table[pci_index])); \
		}                                                                              \
		return pdFALSE;                                                                \
	}

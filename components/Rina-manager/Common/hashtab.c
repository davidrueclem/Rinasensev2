...
static int prvMorefree(hashtab_t *tab, unsigned num2add)
{
	...
	if ((tab->ulMaxEntries && tab->ulCurEntries + num2add > tab->ulMaxEntries) || !(e = (hashent_t *)malloc(size * num2add)))
	{
		return (0);
	}
	for (i = 0; i < num2add; i++)
...
hashtab_t *pxHtNewHashTable(const char *tablename, unsigned initentries, unsigned maxentries, unsigned entryincrement, unsigned numbuckets)
{
	...
	if (tab == NULL || (listheads = (dlList_t *)malloc(sizeof(dlList_t) * numbuckets)) == NULL)
	{
